...
export class AbstractPay {
    ...
    if (Array.isArray(owner)) {
      ...
      this.owners.forEach(({ wallet }: ISmartWallet) => {
        if (this.walletAddress !== wallet.account?.address) {
          throw new Error(
            'All Smart Wallets should be the same wallet on different chains'
          );
        }
      });
    } else {
      ...
      this.walletAddress =
        this.owners[0].wallet.address || this.owners[0].wallet.account.address;
    }
  }
  ...
  // sign Order
  public async signPaymentData(
    typedOrder: ITypedOrder,
    allowanceData?: IDomainData[]
  ) {
    ...
    const signedOrder = await Promise.all(
      this.owners.map(async ({ chainId, wallet }: ISmartWallet) => ({
        chainId,
        data: await wallet.signTypedData(typedOrder),
      }))
    );
      ...
      (await Promise.all(
        allowanceData.map(async (data) => {
          ...
          if (this.isSmartWallet) {
            wallet = this.owners.find(
              (wallet: ISmartWallet) =>
                wallet.chainId === data.domainData.chainId
            );
          } else {
  ...
  public async relayAction(params: RelayParams) {
    try {
      ...
      return apiCall('/relay', 'POST', {
        params: { ...message, chainId: params.chainId, ...sigsplit },
      });
    } catch (error) {
